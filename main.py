...
Code style fixed by Tumanyan A.
"""
...
storage_data:  dict[datetime.time, int] = {}
...
def check_correct_time(time: datetime.time) -> bool:
...
def get_spent_calories(dist: float, current_time: datetime.time) -> float:
    """Получить значения потраченных калорий."""
    time = current_time.hour + current_time.minute / 60
    mean_speed = (dist / time)
    ntime_calories = K_1 * WEIGHT + (mean_speed ** 2 // HEIGHT) * K_2 * WEIGHT
    return ntime_calories * time * 60
...
def show_message(time: datetime.time, steps: int,
                 dist: float, calories: float,
                 achiev: str) -> None:
